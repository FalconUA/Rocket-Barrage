...
Обрати внимание, что из "игрового мира" **WorldSimulator** можно извлечь вектор событии с помощью метода **ExportEvents** (Событии сохранены в виде строки, типа *"Player1 has killed Player2 with a Homing Missile"*), информации игроков с помощью метода **ExportPlayerInfo** (можно получать информацию про кол-во убийств, смертей, нанесённого дамага), а так же параметры всех объектов с помощью метода **GetObjects** (структура **GraphicObject**, я знаю, что эта структура выглядит не эстетично, но, насколько я помню, передавать всё одним вектором вроде легче чем передавать три вектора?). Эти методы нужно вызывать после каждого шага симуляции и мгновенно отсылать игрокам. Как договорились ранее, мы всё будем вычислять на сервере, и в каждый момент времени будем передавать координаты всех объектов. Да, я понимаю что многое из вышеописанных ты уже написал, но, опять же повторюсь, для облегчении работы человека который будет все эти куски собирать в цельную кучу, для облегчении дальнейших работ связанных с улучшением / добавлением новых фичей, и, самое главное, для обеспечение единой физики, единого игровой динамики и в онлайн режиме, и в оффлайн режиме, лучше чтоб игровой движок был единственным. 
...
* **bool AddHomingMissile(sf::Vector2i MousePosition);** // действие игрока, функция будет отсылать запрос серверу
* **bool AddBouncingBomb(sf::Vector2i MousePosition);** // действие игрока, функция будет отсылать запрос серверу
* **bool AddGrenade(sf::Vector2i MousePosition);** // действие игрока, функция будет отсылать запрос серверу
...
* **std::vector< std::string > ExportEvents();** // никакой запрос не отсылаем серверу, а просто извлекаем информацию от вышеописанных векторов
* **std::vector< rbw::PlayerExportInformation > ExportPlayerInfo();** // никакой запрос не отсылаем серверу, а просто извлекаем информацию от вышеописанных векторов
* **bool GetObjects(std::vector< GraphicObject >* objects);** // никакой запрос не отсылаем серверу, а просто извлекаем информацию от вышеописанных векторов
